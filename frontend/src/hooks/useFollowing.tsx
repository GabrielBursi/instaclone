...
export function useFollowing(userId?: number) {
	...
	const {
		...
		onMutate: async (variables) => {
			await queryClient.cancelQueries({ queryKey: ["user", variables.following_id] });

			const previousUserData = queryClient.getQueryData(["user", variables.following_id]);

			queryClient.setQueryData(["user", variables.following_id], (old: OldUser) => {
				if (!old) return old;
				return {
					...old,
			...
			queryClient.setQueryData(["user", variables.follower_id], (old: OldUser) => {
				if (!old) return old;
				return {
					...old,
		...
		onError: (_err, _variables, context) => {
			if (context?.targetUserId && context.previousUserData) {
				queryClient.setQueryData(["user", context.targetUserId], context.previousUserData);
			}
		},
		onSettled: async (_data, _err, variables) => {
			await Promise.all([
				queryClient.invalidateQueries({ queryKey: ["user", variables.following_id] }),
				queryClient.invalidateQueries({ queryKey: ["user", variables.follower_id] }),
