...
export function usePosts(userId?: number) {
	...
	usePrefetchInfiniteQuery({
		...
		getNextPageParam: (lastPage) => {
			...
			if (next_cursor) return next_cursor.toString()
			return undefined
		},
		...
		onMutate: async (newPost) => {
			...
			queryClient.setQueryData(['posts', userId], (old: OldPosts) => {
				...
				newPages[0] = {
					...
					data: [
						optimisticPost,
						...(newPages[0]?.data ? newPages[0].data : []),
					],
				}
	...
	const [postsMeta, setPostsMeta] = useState<SafeType<MetaPagination> | null>(
		null
	)
	...
	useEffect(() => {
		...
		if (postsData) {
			const validPages = postsData.pages.filter(
				(p): p is SafeType<PaginatedResponde<Post>> => !(p instanceof Error)
			)
	...
	return {
		...
		createPostIsLoading,
		createdPost:
			!!createdPost && !(createdPost instanceof Error) ? createdPost : null,
	} as const
}
